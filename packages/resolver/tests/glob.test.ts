...
// We will use a temporary directory for real filesystem tests for readDirFn
...
describe("@kirejs/resolver", () => {
    ...
    test("should correctly list files using readDirFn with pattern", async () => {
        const kire = new Kire();
        kire.plugin(KireResolver);

        if (!kire.readDirFn) {
            throw new Error("readDirFn not initialized");
        }

        // We need to change directory or use absolute paths for glob matching relative to CWD
        // But our implementation uses recursion.
        // Let's see how we implemented readDirFn. It uses `.` as root by default.
        ...
        const files = await kire.readDirFn("package.json");
    ...
    test("should match glob patterns recursively", async () => {
        ...
        if (kire.readDirFn) {
            const files = await kire.readDirFn("packages/resolver/src/*.ts");
            expect(files.length).toBeGreaterThan(0);
            expect(files.some(f => f.endsWith("index.ts"))).toBeTrue();
        } else {
            throw new Error("readDirFn not defined");
        }
    });
});
