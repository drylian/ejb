    ...
    load(kire: Kire, opts) {
        ...
        kire.directive({
            ...
            onCall(ctx) {
                ...
                ctx.res(`     const withClass = svg.replace('<svg', '<svg class="' + cls + '"');`);
        ...
        kire.element('iconify', async (ctx) => {
            const iconName = ctx.element.attributes.i || ctx.element.attributes.icon;
            if (!iconName) {
                ctx.update('<!-- <iconify> missing "i" or "icon" attribute -->');
                return;
            }

            const className = ctx.element.attributes.class || ctx.element.attributes.className || defaultClass;
