import type { Kire, KirePlugin } from 'kire';
import { join, dirname, extname, resolve, relative, basename } from 'path';
import { readdir, mkdir, writeFile, readFile } from 'fs/promises';
import { createServer } from 'http';
import { assetsCache } from '@kirejs/assets';
import { glob } from 'glob';

declare module 'kire' {
    interface Kire {
        parseMarkdown?(content: string): Promise<string>;
    }
}

// Singleton instance to hold Kire reference
let kireInstance: Kire | null = null;
let assetsPrefix = '_kire';

interface SsgOptions {
    assetsPrefix?: string;
}

interface BuildOptions {
    out: string;
    dir?: string; // Source directory relative to root, defaults to root
}

export const KireSsg: KirePlugin<SsgOptions> & { 
    build: (opts: BuildOptions) => Promise<void>;
    dev: (opts?: { port?: number }) => Promise<void>;
} = {
    name: '@kirejs/ssg',
    options: {},
    load(kire: Kire, opts) {
        kireInstance = kire;
        if (opts?.assetsPrefix) {
             assetsPrefix = opts.assetsPrefix.replace(/^\//, '').replace(/\/$/, '');
        }
    },

    async build(opts: BuildOptions) {
        if (!kireInstance) throw new Error("KireSsg plugin not registered or Kire instance not ready.");
        
        const outDir = resolve(opts.out);
        const rootDir = resolve(kireInstance.root);
        
        console.log(`Building to ${outDir}...`);
        
        await mkdir(outDir, { recursive: true });
        
        // Helper to crawl
        async function getFiles(dir: string): Promise<string[]> {
            const dirents = await readdir(dir, { withFileTypes: true });
            const files = await Promise.all(dirents.map(async (dirent) => {
                const res = resolve(dir, dirent.name);
                if (dirent.isDirectory()) {
                    return getFiles(res);
                } else {
                    return res;
                }
            }));
            return Array.prototype.concat(...files);
        }

        const allFiles = await getFiles(rootDir);
        const ext = kireInstance.extension.startsWith('.') ? kireInstance.extension : `.${kireInstance.extension}`;
        
        const templateFiles = allFiles.filter(f => f.endsWith(ext));

        for (const file of templateFiles) {
            const relativePath = relative(rootDir, file);
            if (relativePath.split('/').some(p => p.startsWith('_'))) continue;

            try {
                // 1. Render the template to check for generator markers
                // We pass an empty object or maybe we should detect params?
                // For generator detection, we render without specific locals first.
                const html = await kireInstance.render(file);
                
                // Check for marker
                const markerRegex = /<!-- KIRE_MARKDOWN_GEN:(.*?) -->/;
                const match = html.match(markerRegex);

                if (match) {
                    const globPattern = match[1];
                    console.log(`> Detected Markdown Generator in ${relativePath} for '${globPattern}'`);
                    
                    // Find markdown files
                    // Glob is relative to root?
                    const mdFiles = await glob(globPattern, { cwd: rootDir });
                    
                    for (const mdFile of mdFiles) {
                        // mdFile can be Path object or string depending on glob version/options. 
                        // With just cwd option, it usually returns strings relative to cwd.
                        const mdRelative = String(mdFile); 
                        const mdPath = resolve(rootDir, mdRelative);
                        const mdContent = await readFile(mdPath, 'utf-8');

                        if (typeof kireInstance.parseMarkdown !== 'function') {
                             throw new Error("Markdown plugin not loaded or parseMarkdown method missing.");
                        }

                        const parsedContent = await kireInstance.parseMarkdown(mdContent);
                        
                        // Render template again with content
                        // We assume the template uses {{ content }} or similar variable, 
                        // OR the @markdown directive logic in runtime handles injection if we pass context?
                        // Wait, @markdown directive in "glob mode" just printed the marker.
                        // It didn't render content.
                        // So we need the template to be able to accept 'content' variable and print it?
                        // But the @markdown directive occupied the place where content should go.
                        
                        // IMPORTANT: The template has `@markdown('docs/*.md')`.
                        // This resulted in `<!-- KIRE_MARKDOWN_GEN... -->`.
                        // We want to replace that marker with the actual content!
                        // So we can just replace the marker in the already rendered string 'html'.
                        // AND we might need to pass other metadata (frontmatter?)
                        
                        // Simple approach: Replace marker with content.
                        // But we need to generate a NEW file for each markdown.
                        
                        const finalHtml = html.replace(match[0], parsedContent as string);
                        
                        // Determine output path
                        // mdRelative is already defined above
                        // e.g. docs/intro.md
                        // Output: dist/docs/intro.html
                        const htmlOutPath = mdRelative.replace(/\.(md|markdown)$/, '.html');
                        const fullOutPath = join(outDir, htmlOutPath);
                        
                        await mkdir(dirname(fullOutPath), { recursive: true });
                        await writeFile(fullOutPath, finalHtml);
                        console.log(`  -> Generated ${htmlOutPath}`);
                    }

                } else {
                    // Normal file
                    const htmlPath = relativePath.replace(new RegExp(`\\${ext}$`), '.html');
                    const outPath = join(outDir, htmlPath);
                    
                    await mkdir(dirname(outPath), { recursive: true });
                    await writeFile(outPath, html);
                    console.log(`✓ ${htmlPath}`);
                }

            } catch (e: any) {
                console.error(`✗ Failed to render ${relativePath}:`, e.message);
            }
        }

        // Write assets
        if (assetsCache.size > 0) {
            const assetsDir = join(outDir, assetsPrefix);
            await mkdir(assetsDir, { recursive: true });
            
            for (const [hash, asset] of assetsCache) {
                const filename = `${hash}.${asset.type}`;
                await writeFile(join(assetsDir, filename), asset.content);
                console.log(`✓ Asset: ${assetsPrefix}/${filename}`);
            }
        }
        
        console.log("Build complete.");
    },

    async dev(opts = {}) {
        if (!kireInstance) throw new Error("KireSsg plugin not registered.");
        
        const port = opts.port || 3000;
        
        const server = createServer(async (req, res) => {
            const url = req.url || '/';
            console.log(`[DEV] Request: ${req.method} ${url}`);

            try {
                // Handle assets
                const prefixPath = `/${assetsPrefix}/`;
                if (url.startsWith(prefixPath)) {
                    console.log(`[DEV] Attempting to serve asset: ${url}`);
                    const match = url.match(/\/([a-f0-9]{8})\.(js|css)$/);
                    if (match) {
                        const hash = match[1];
                        const ext = match[2] as 'js' | 'css';
                        const asset = assetsCache.get(hash);
                        if (asset && asset.type === ext) {
                            res.setHeader('Content-Type', ext === 'js' ? 'application/javascript' : 'text/css');
                            res.end(asset.content);
                            console.log(`[DEV] Served asset: ${url}`);
                            return;
                        }
                    }
                    // If match failed or asset not found in cache
                    console.warn(`[DEV] Asset not found in cache: ${url}`);
                    res.statusCode = 404;
                    res.end('Asset Not Found');
                    return;
                }

                // Handle pages
                let baseFilename = url === '/' ? 'index' : url.substring(1);
                if (baseFilename.endsWith('.html')) baseFilename = baseFilename.slice(0, -5);
                
                const candidates = [
                    baseFilename,
                    `${baseFilename}/index`,
                    `pages/${baseFilename}`,
                    `pages/${baseFilename}/index`
                ];

                let html: string | null = null;
                let servedCandidate = '';

                for (const candidate of candidates) {
                    try {
                        // console.log(`[DEV] Trying candidate: ${candidate}`);
                        html = await kireInstance!.render(candidate);
                        servedCandidate = candidate;
                        break;
                    } catch (e: any) {
                        if (e.message.includes('No resolver') || e.message.includes('ENOENT') || e.message.includes('Template not found')) {
                            continue;
                        }
                        throw e; // Re-throw unexpected errors
                    }
                }
                
                if (html !== null) {
                    res.setHeader('Content-Type', 'text/html');
                    res.end(html);
                    console.log(`[DEV] Served template: ${servedCandidate} (Request: ${url})`);
                } else {
                    console.warn(`[DEV] Template not found (404): ${url}. Tried: ${candidates.join(', ')}`);
                    res.statusCode = 404;
                    res.end(`Not Found: ${req.url}`);
                }
                
            } catch (e: any) {
                if (e.message.includes('No resolver') || e.message.includes('ENOENT') || e.message.includes('Template not found')) {
                    console.warn(`[DEV] Template not found (404): ${req.url}`);
                    res.statusCode = 404;
                    res.end(`Not Found: ${req.url}`);
                } else {
                    console.error(`[DEV] Internal Server Error (500) for ${req.url}:`, e);
                    res.statusCode = 500;
                    res.end('Internal Server Error');
                }
            }
        });

        server.listen(port, () => {
            console.log(`Server running at http://localhost:${port}`);
        });
        
        return new Promise(() => {});
    }
};

export default KireSsg;