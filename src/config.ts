...
export function makeConfig(directives: Record<string, EjbDirectivePlugin>, options: MakeConfigOptions) {
    const configDirectives = Object.values(directives).map(dir => {
    ...
    return {
        $schema: options.$schema ?? "https://raw.githubusercontent.com/drylian/ejb/main/vs-code/schemas/ejbconfig.schema.json",
