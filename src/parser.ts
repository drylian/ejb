...
import { DIRECTIVE_REGEX, EJB_DEFAULT_PREFIX_VARIABLE, EjbAst } from "./constants";
...
export function ejbParser<A extends boolean>(ejb: Ejb<A>, template: string): RootNode {
    ...
    while (cursor < template.length) {
        ...
        // Handle directives
            ...
            if (name === 'end') {
                if (stack.length === 1) throw new Error(`Unexpected ${directivePrefix}end directive`);
                stack.pop();
            } 
            // Handle regular directives and sub-directives
                ...
                for (let i = stack.length - 1; i >= 0; i--) {
                    ...
                    if (potentialParent.type === EjbAst.Directive || potentialParent.type === EjbAst.SubDirective) {
                ...
                if (!directiveDef) {
                    throw new Error(`[EJB] Directive not found: ${directivePrefix}${name}`);
                }
    ...
    while (stack.length > 1) {
        ...
        if (node && (node.type === EjbAst.Directive || node.type === EjbAst.SubDirective)) {
