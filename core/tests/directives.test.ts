...
describe("Kire Native Directives", () => {
  ...
  describe("Control Flow", () => {
    ...
    it("nested @if", async () => {
        ...
        expect(await render(tpl, { outer: true, inner: true })).toBe("OuterInner");
...
describe("Kire Component Directives", () => {
  ...
  kire.resolverFn = async (path) => {
    if (path.includes('alert')) return `<div class="alert {{ it.type }}">{{ it.slots.default }} @if(it.slots.footer)<footer>{{ it.slots.footer }}</footer>@end</div>`;
    if (path.includes('card')) return `<div class="card"><h3>{{ it.title }}</h3>{{ it.slots.default }}</div>`;
    return null;
  };
  ...
  it("@component with complex variables", async () => {
    ...
    const html = await render(tpl, { user: { name: 'World', status: 'success' } });
  ...
  it("@component without provided slots (should render default slot if used)", async () => {
    ...
    const html = await render(tpl);
    expect(html).toContain('<div class="card"><h3>My Card</h3>No custom slots</div>');
  });
...
describe("Kire Include Directive", () => {
    ...
    kire.resolverFn = async (path) => {
        ...
        if (path.includes('grandchild')) return `Grandchild: {{ it.item.name }} and {{ it.item.value }}`;
        if (path.includes('nested')) return `@include('grandchild', { item: it.n_item })`;
        return null;
    };
    const render = (tpl: string, locals = {}) => kire.render(tpl, locals);
    ...
    it("@include passing complex locals", async () => {
        ...
        const html = await render(tpl, { user: { firstName: 'John', lastName: 'Doe' } });
