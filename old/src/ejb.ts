...
import { Ejb, type EjbDirectivePlugin, type EjbDirectiveParent } from '../../src';
...
export const createEJB = (output_channel: vscode.OutputChannel, enriched_directives: EnrichedDirective[]) => {
    const ejb = new Ejb();
    
    const virtual_directives: Record<string, EjbDirectivePlugin> = {};

    for (const enriched_directive of enriched_directives) {
        if (validateDirective(enriched_directive, output_channel)) {
            ...
            virtual_directives[enriched_directive.name] = directive_plugin;
        }
    }
